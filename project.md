...
分析并解构 xv6 RISC-V 源码，利用其源码(重写，并非直接复制然后进行修改)创建能够完成以下任务的操作系统内核。采用影响最小的方式进行每个项目的开发，项目采用增量开发方式，每个任务在上一任务基础上添加新功能，即直接复制上一个项目到当前项目，然后增加代码。

主机是arm 64环境,你可以直接运行make命令：make qemu TOOLPREFIX=riscv64-linux-gnu- QEMU=qemu-system-riscv64
，然后查看运行结果进而完成修改。

注意：你在编写时需要添加大量的注释：说明你每一行代码的作用
...
每个任务完成后，需要编写详尽的实验报告。注意编写时不能使用emoji表情。
