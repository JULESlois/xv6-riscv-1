...
main(void)
{
  ...
  char *argv[] = { "test_sync", 0 }; // exec argument vector terminated by a null pointer.
  ...
  if(pid == 0){                    // The child process sees fork return zero.
    exec("test_sync", argv);          // Replace the child image with the test ELF from fs.img.
    printf("init: exec test_sync failed\n"); // exec returns only on failure.
    exit(1);                       // Report the exec failure.
  }

  wait(&status);                   // Wait for the test process to finish.
  printf("init: test_sync exited with status %d\n", status); // Print the test exit status.
